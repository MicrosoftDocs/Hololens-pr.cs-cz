...
ms.locfileid: "126032209"
---
# <a name="known-issues-for-hololens-1st-gen"></a>Známé problémy HoloLens (1. generace)

Tady je aktuální seznam známých problémů s HoloLens zařízeními. Nejprve zkontrolujte, jestli se vám nesnídí chování. Tento seznam se bude aktualizovat při zjištěných nebo ohlášených nových problémech nebo při řešení problémů v budoucnu HoloLens aktualizace softwaru.
...
- Na stránce Virtuální vstup nejsou ovládací prvky Gesture a Scroll v části Virtuální gesta funkční. Jejich použití nebude mít žádný vliv. Virtuální klávesnice na virtuální vstupní stránce funguje správně.

- Po povolení režimu pro vývojáře Nastavení může trvat několik sekund, než se přepínač zapne a Portál zařízení povolí.
...
- Pokud je pro vaši firmu přijatelné, je u uživatelských účtů Microsoft podporováno automatické nahrávání fotoaparátu. Ke svému pracovnímu nebo školnímu účet Microsoft přihlášení (aplikace OneDrive podporuje duální přihlašování) se můžete přihlásit ke svému pracovnímu nebo školnímu účtu. Ve svém účet Microsoft v rámci OneDrive můžete povolit automatické nahrávání fotoaparátů na pozadí.
...
1. Jakmile se nástroj ukončí bez jakýchkoli zpráv (mělo by to trvat jen pár sekund), budete teď moct nasadit a ladit z Visual Studio 2017 nebo novější verze.  Průběžné používání nástroje není nutné.
...
Při pokusu o spuštění aplikace a Microsoft Store na počítači může docházet k HoloLens. Zjistili jsme, že k problému dochází, když aktualizace aplikací na pozadí nasadí novější verzi balíčků architektury v určitých sekvencích, zatímco jedna nebo více jejich závislých aplikací stále běží. V tomto případě automatická aktualizace aplikace doručila novou verzi rozhraní .NET Native Framework (verze 10.0.25531 až 10.0.27413) způsobila, že aplikace, které běží, se správně ne aktualizují pro všechny spuštěné aplikace využívající předchozí verzi rozhraní.  Postup aktualizace architektury je následující:
...
Někteří uživatelé oznámili, že zavřením zamkřených aplikací a spuštěním jiných aplikací, jako jsou Centrum Feedback, 3D prohlížeč nebo Fotky, se problém vyřeší, ale ve 100 % případů to nefunguje.
...
Stejně jako v případě každé HoloLens operačního systému jsme image FFU také zveřejnili na [webu Microsoft Download Center](https://aka.ms/hololensdownload/10.0.17763.380).

Pokud 29. 3. 2019 2019 2018 2016 2016 chyste se chyste aktualizace, vydali jsme novou verzi aplikace Microsoft Store UPW. Po aktualizaci verze Storu:
...
1. Zobrazí se obrazovka podobná následující.  Chcete přejít do části Instalace  aplikace a přejít na místo, kde jste tyto dva soubory APPX rozbalili. Můžete provést pouze jednu po jedné, takže po výběru prvního z nich klikněte v části Nasadit na Přejít. Pak to proveďte pro druhý soubor APPX.

   ![Windows Portál zařízení k instalaci Side-Loaded aplikace.](images/20190322-DevicePortal.png)
...
- Aktuální [verzi nástroje najdete](/windows/mixed-reality/install-the-tools) v tématu Instalace nástrojů Visual Studio která se doporučuje pro HoloLens vývoj.
...
- Pokud aplikace nastaví zaměřovací [bod](/windows/mixed-reality/focus-point-in-unity) za uživatelem nebo normální hodnotu na fotoaparát.forward, hologramy se nezobrazí na Záznam hybridní reality fotografiích nebo videích. Dokud se tato chyba v nástroji Windows nevyřešila, měly by aplikace aktivně nastavit fokus, aby se zajistilo, že je rovina v normálu nastavená proti směru fotoaparátu (například normální = -camera.forward). [](/windows/mixed-reality/focus-point-in-unity)
...
- Pokud restartujete počítač HoloLens bezdrátového ovladače Xbox, nebude se kontroler automaticky znovu připojovat k HoloLens. Světlo tlačítka Guide (Průvodce) bude pomalu blikat, dokud se kontroler po 3 minutách nesvítí. Pokud chcete kontroler okamžitě znovu připojit, vypněte ho tak, že podržíte tlačítko Guide (Průvodce), dokud se světlo nevypíná. Po opětovném zapnutí kontroleru se kontroler znovu připojí k HoloLens.

- Pokud se HoloLens bezdrátového ovladače Xbox připojí do pohotovostního režimu, všechny vstupy na kontroleru probudí HoloLens. Tomu můžete zabránit vypnutím kontroleru, až ho budete používat.
