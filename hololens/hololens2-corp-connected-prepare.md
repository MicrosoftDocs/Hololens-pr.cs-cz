...
> [!NOTE]
> I když HoloLens 2 nasazená a spravovaná jako mobilní zařízení, obvykle se používá jako sdílené zařízení mezi mnoha uživateli.
...
## <a name="pkcs"></a>PKCS
Intune také podporuje použití certifikátů PKCS (private and public key pair). Další informace najdete v části Použití certifikátů [privátních Microsoft Intune](/mem/intune/protect/certificates-pfx-configure) veřejných klíčů v nástroji .

## <a name="proxy"></a>Proxy server
Většina podnikových intranetových sítí ke správě externích přenosů využívá proxy server. Pomocí HoloLens 2 můžete nakonfigurovat síť proxy server připojení k síti Ethernet, Wi-Fi a VPN.

Existuje několik různých typů proxy serveru a způsobů konfigurace proxy serveru. Pro účely této příručky se rozhodneme zvolit **proxy wi-fi,** nastavit prostřednictvím adresy URL PAC a nasadit ji přes MDM. To má tu výhodu, že se nasazovat automaticky přes MDM, protože je možné aktualizovat soubor PAC místo konfigurace server:port a nakonec pomocí proxy serveru Wi-Fi nakonfigurovat proxy server tak, aby se používal jenom pro jedno připojení Wi-Fi, což umožňuje, aby se zařízení i nadále používala, pokud jsou připojená v jiném umístění.
