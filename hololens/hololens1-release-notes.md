...
description: Přečtěte si o aktualizacích v každé nové HoloLens verzi.
author: evmill
ms.author: v-evmill
manager: yannisle
ms.prod: hololens
...
Pro vývojáře to znamená, HoloLens aplikace (1. generace) nebudou rozhraní OPENXR API podporovat.  Tyto náhlavní soupravy zůstávají podporované v Unity 2019 LTS s back-endem rozhraní WinRT API po celý životní cyklus Unity 2019 LTS až do poloviny roku 2022.
...
| **Zastavení zachytávání videí z nabídky Spustit nebo Rychlé akce** | Pokud spustíte zaznamenávání videa z nabídka Start nebo rychlé akce, budete moct nahrávání zastavit ze stejného místa. (Nezapomeňte, že to můžete dělat i pomocí hlasových příkazů.) |
| **Project k zařízení Miracast s podporou připojení** | Project adaptér Microsoftu HoloLens obsah vašeho počítače na nejbližší zařízení Surface nebo na tv nebo monitor.  V **nabídce** Start **vyberte Připojení** a pak vyberte zařízení, pro které chcete projektovat. **Poznámka:** Můžete nasadit HoloLens použití Miracast projekce bez povolení vývojářského režimu. |
...
| **Sdílení v okolí** | Přidání možnosti Windows možnosti Sdílení v okolí, což vám umožní sdílet zachytávání s blízkým Windows zařízením. Když pořizovat fotku nebo video na HoloLens (nebo použít tlačítko Sdílet z aplikace, jako je Microsoft Edge), vyberte nejbližší zařízení Windows se kterým chcete sdílet. |
...
| Nastavení HoloLens zařízení pomocí MDM (přejmenování) | Správci IT můžou zobrazit a přejmenovat HoloLens zařízení ve své konzole MDM. Pokyny a dostupnost funkcí najdete v dokumentaci k MDM. |
...
- Když vytvoříte místní účet ve zřizovacím balíčku, platnost hesla už nevyprší každých 42 dnů.
...
- Dříve jste se po přihlášení k zařízení pomocí účtu Azure Active Directory (Azure AD)  museli  k získání přístupu k podnikovým prostředkům Nastavení přístup k pracovnímu přístupu. Teď se přihlásíte pomocí účtu Azure AD a registrace probíhá automaticky.

- Před přihlášením můžete zvolit ikonu sítě pod polem hesla a zvolit jinou síť, Wi-Fi k ní chcete připojit. Můžete se také připojit k síti hostů, jako je hotel, konferenční centrum nebo firma.
